...
import { Card, SearchInput, EmptyState, useToast, Stack, Text, Button, Tooltip, Label, Icon, Select, Badge } from '@pulwave/ui';
import type { ButtonKind } from '@pulwave/ui';
import type { IconComponent } from '@pulwave/ui/icon-library';
import * as AllIcons from '@pulwave/ui';
...
export const IconLibrary = () => {
    ...
    const [color, setColor] = useState('var(--color-primary-500)'); // Primary color as default

    const resetAppearance = () => {
        setSize(24);
        setStrokeWidth(2);
        setColor('var(--color-primary-500)'); // Reset to primary
    };
    ...
    const iconList = useMemo(() => {
        return Object.entries(AllIcons)
            .filter(([name, component]) => {
                ...
                // Also exclude internal exports if user defined any non-icon exports in ui2
                return (typeof component === 'object' || typeof component === 'function') && name !== 'default';
            })
    ...
    const filteredIcons = useMemo(() => {
        ...
        if (selectedCategory !== 'All') {
            const categoryIcons = CATEGORIES[selectedCategory as keyof typeof CATEGORIES] || [];
    ...
    return (
        <Stack gap="xl">
            ...
            <div className="icon-library-page__title-row">
                <Badge variant="medium" status="neutral" size="m">{iconList.length} icons</Badge>
                <Text category="body" size="l" className="text-secondary">
                    ...
                    <div className="icon-library-page__appearance-stack">
                        ...
                        <div>
                            ...
                            <div className="icon-library-page__color-grid">
                                {[
                                    { kind: 'primary', color: 'var(--color-primary-main)' },
                                    { kind: 'secondary', color: 'var(--color-secondary-main)' },
                                    { kind: 'tertiary', color: 'var(--color-tertiary-main)' },
                                    { kind: 'neutral', color: 'var(--color-neutral-900)' },
                                    { kind: 'success', color: 'var(--color-feedback-success)' },
                                    { kind: 'warning', color: 'var(--color-feedback-warning)' },
                                    { kind: 'error', color: 'var(--color-feedback-error)' },
                                    { kind: 'info', color: 'var(--color-feedback-info)' },
                                ].map((c) => (
                                    <Tooltip key={c.kind} content={c.kind.charAt(0).toUpperCase() + c.kind.slice(1)} direction="top">
                                        <Button
                                            ...
                                            variant="filled"
                                            size="m"
                                            className={color === c.color ? 'button--active' : ''}
                        ...
                        {filteredIcons.map(([name, IconComponent]) => (
                            <Tooltip key={name} content={name} direction="top">
                                <Button
                                    ...
                                    variant="outlined"
                                    shape="circle"
                                    size={getButtonSizeForIcon(size)}
                                    onClick={() => handleCopy(name)}
                                    className="icon-library-page__icon-button"
                                    style={{
                                        ...(color ? { borderColor: color, color: color } : {}),
